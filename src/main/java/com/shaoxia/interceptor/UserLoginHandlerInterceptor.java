package com.shaoxia.interceptor;
...
public class UserLoginHandlerInterceptor implements HandlerInterceptor{
	...
	private String[] allowUrls;//不拦截的路径
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		//获取请求路径(http://www.tiaoxiaopi.com/admin/admin-index.html)[/admin/admin-index.html]
		String requestUrl = request.getRequestURI().replace(request.getContextPath(), "");
		//排除不拦截的路径
		if(allowUrls!=null && allowUrls.length>=1){
			for(String url:allowUrls){
				if(StringUtils.equals(url, requestUrl)){
					return true;
				}
			}
		}
